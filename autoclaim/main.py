...
try:
    ...
    # Extract path and messages from config
    config_path = config.get("path", os.path.join(application_path, "config.json"))
    success_message = config["message"]["success"]
    file_not_found_message = config["message"]["file_not_found"].format(path=config_path)
    json_decode_error_message = config["message"]["json_decode_error"]

    color_success = config["color"]["success"]
    color_error = config["color"]["error"]
    color_reset = config["color"]["reset"]

    # Print success message
    print(color_success + success_message + color_reset)

    try:
        with open(config_path, 'r') as file:
            data = json.load(file)
            print(color_success + success_message + color_reset, data)
    except FileNotFoundError:
        print(color_error + file_not_found_message + color_reset)
    except json.JSONDecodeError as e:
        print(color_error + json_decode_error_message.format(path=config_path, error=e) + color_reset)
...
class BLUMIDIOT:
    ...
    def playgame(self, access_token):
        ...
        for i in range(play):
            ...
            if "OK" in res.text:
             ...
             self.game_counter = 0
            self.game_counter += 1
            self.log(f"{hijau}Success: Earned {merah}{point} {putih}points from game! Tickets used: {biru}{self.game_counter}")
    ...
    def countdown(self, t):
        while t:
            ...
            print(f"{putih}menunggu proses auto verifyhuman {jam}:{menit}:{detik} ", flush=True, end="\r")
            t -= 1
            time.sleep(1)
    ...
    def main(self):
        banner = f"""
    {hijau}AUTO CLAIM  {putih}BLUM
    
    {hijau}Message : {putih} Resiko Ditanggung Sendiri ya, {biru} So this NETRALLIZION!, Beware The {putih}BATBLUM
        """
